...
class DVDGayOnlineIE(SeleniumInfoExtractor):
    ...
    def _get_entry_realgalaxy(self, url, postid, nplayer, _pre):
        ...
        try:
            ...
            try:
                if not driver:
                    raise_extractor_error('driver error init')
                else:
                    with self.get_har_logs('gvdgayonline', videoid=postid, msg=pre, port=_port) as harlogs:
                        ...
                        if not (_player := try_call(lambda: _players[nplayer - 1])) or 'realgalaxy' not in _player.text:  # type: ignore
                            ...
                            if isinstance(_res, dict) and (_error := _res.get('error')):
            ...
            if not (
                ...
                videoid = traverse_obj(try_get(
                    re.search(r'https?://%s/e/(?P<id>[\dA-Za-z]+)(\.html)?' % dom, urlembed),
                    lambda x: x.groupdict()), 'id')
                if not videoid:
                ...
                if not isinstance(_title, str):
                    ...
                    _temp = _title
                    _temp = try_get(re.findall(r'(1080p|720p|480p)', _temp), lambda x: _temp.split(x[0])[0]) or _temp
                    _title = re.sub(r'(\s*-\s*202)', ' 202', _temp).replace('mp4', '').replace('mkv', '').strip(' \t\n\r\f\v-_')
                ...
                if not (m3u8_doc := (m3u8_doc or try_get(self._send_request(m3u8_url, headers=_headers), lambda x: x.text))):
                    raise_reextract_info(f'{pre} Couldnt get video info')
                else:
                    _formats = []
                    _subtitles = {}
    ...
    def _get_entry(self, url, **kwargs):
        ...
        if not webpage or any([_ in webpage for _ in ('<title>Server maintenance', '<title>Video not found')]):
            ...
            players = {}
            if _servers := get_elements_by_class('server', webpage):
                players = {el: i + 1 for i, el in enumerate(map(lambda x: x.split('.')[0] if x else x, _servers))}
            if not players:
