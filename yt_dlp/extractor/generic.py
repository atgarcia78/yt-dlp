...
class GenericIE(InfoExtractor):
    ...
    def _real_extract(self, url):
        ...
        if streamtape_url:
            return self.url_result(streamtape_url, ie=StreamtapeIE.ie_key())
        ...
        if not found:
            ...
            if found:
                ...
                flashvars = re.search(r'(?ms)<script.*?>.*?var\s+flashvars\s*=\s*(\{.*?\});.*?</script>', webpage) or re.search(r'(?ms)var\s+flashvars\s*=\s*([^;]+);',webpage)
