...
from .commonwebdriver import dec_on_exception, ExtractorError, SeleniumInfoExtractor, limiter_5, By, copy
import re
...
class DoodStreamIE(SeleniumInfoExtractor):
    ...
    def _extract_urls(webpage):
        return [mobj.group('url') for mobj in re.finditer(r'<iframe[^>]+?src=([\"\'])(?P<url>https?://(?:www\.)?dood(?:stream)?\.[^/]+/[ed]/[a-z0-9]+)\1',webpage)]

    @dec_on_exception
    @limiter_5.ratelimit("doodstream", delay=True)  
    def _get_info_for_format(self, url, headers=None, msg=None):
        
        if msg: pre = f'{msg}[get_video_info]'
        else: pre = '[get_video_info]'
        self.logger_debug(f"{pre} {self._get_url_print(url)}")
        return super().get_info_for_format(url, headers=headers)
    ...
    def _get_entry(self, url, check_active=False, msg=None):
        ...
        try:
            ...
            headers = copy.deepcopy(self._FIREFOX_HEADERS)
            
            headers.update({'Referer': 'https://dood.to/'})
            ...
            if check_active:
                _videoinfo = self._get_info_for_format(video_url, headers=headers, msg=pre)
