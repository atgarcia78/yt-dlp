        ...
        try:
            if not driver:
                ...
                if not videoid:
                    ...
                    if not isinstance(el_video, WebElement):
                        raise_extractor_error('no el video')
                    else:
                        if _poster := el_video.get_attribute('poster'):
                ...
                if not isinstance(video_url := self.wait_until(driver, 30, getvideourl()), str):
                    raise ExtractorError("no video url")
                else:
                    self.logger_debug(f"{_pre} video_url [{video_url}]")
                    if not (title := (
                        ...
                        if el_quality:
                            ...
                            for _el in el_quality:
                                ...
                                assert _quality
                                self.write_debug(f"quality:[{_quality}]")
                                ...
                                if not _info_video:
                                    self.report_warning(f"{_pre} {_format_id} no video info")
                                else:
